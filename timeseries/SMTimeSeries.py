""" Module for Storage Manager based TimeSeries.
This module defines the SMTimeSeries class.

Classes
-------
SMTimeSeries(SizedContainerTimeSeriesInterface):
    Define the SMTimeSeries class for storage manager based TimeSeries.

Methods
-------
__init__(self, times, values, fsm, id = None):
...
__setitem__(self, index, value):
    Set the value at the index time by value of a SMTimeSeries
    Update the values numpy array and store the SMTimeSeries
    Raise "ValueError" when the index is out of boundary,
...
__add__(self, other):
    ...
    Return a new SMTimeSeries instance with the values being sum of both
    timeseries values
    Raise "TypeError" when other is not an instance of SMTimeSeries class,
    and raise "ValueError" when self's and other's times are not element-wise
    equal within a tolerance
__sub__(self, other):
    Subtract by other's value at each time point

    other: another SMTimeSeries instance
    Return a new SMTimeSeries instance with the values calculated by subtracting
    other's values from self's values
    Raise "TypeError" when other is not an instance of SMTimeSeries class,
    and raise "ValueError" when self's and other's times are not element-wise
    equal within a tolerance
__mul__(self, other):
    ...
    other: another SMTimeSeries instance
    Return a new SMTimeSeries instance with the values calculated by multiplying
...
__neg__(self):
    Return a new SMTimeseries instance with the opposite number of value at each
    time point
__pos__(self):
...
class SMTimeSeries(SizedContainerTimeSeriesInterface):
    ...
    __add__(self, other):
        ...
        timeseries values
        Raise "TypeError" when other is not an instance of SMTimeSeries class,
        and raise "ValueError" when self's and other's times are not
        element-wise equal within a tolerance
    __sub__(self, other):
        Subtract by other's value at each time point

        other: another SMTimeSeries instance
        Return a new SMTimeSeries instance with the values calculated by
        subtracting other's values from self's values
        Raise "TypeError" when other is not an instance of SMTimeSeries class,
        and raise "ValueError" when self's and other's times are not
        element-wise equal within a tolerance
    __mul__(self, other):
        ...
        other: another SMTimeSeries instance
        Return a new SMTimeSeries instance with the values calculated by
        multiplying self's and other's values
        Raise "TypeError" when other is not an instance of SMTimeSeries class,
        and raise "ValueError" when self's and other's times are not
        element-wise equal within a tolerance
    __eq__(self, other):
        ...
        element-wise equal within a tolerance
    __abs__(self):
        Return the L^2 norm of values(the root square of the sum of the
        squared values)
    __bool__(self):
        Return true if the L^2 norm of values is non-zero, false if it's zero
    __neg__(self):
        Return a new SMTimeseries instance with the opposite number of value
        at each time point
    __pos__(self):
        Return a new SMTimeseries instance with the same value at each time point
